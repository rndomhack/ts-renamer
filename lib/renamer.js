...
class Renamer {
    ...
    async checkTime() {
        ...
        const actualDuration = Math.floor((end - start + (start > end ? 0x200000000 : 0)) / 90);
        const programDuration = this.program.endTime.getTime() - this.program.startTime.getTime();

        if (actualDuration < programDuration) {
            throw new Error(`Invalid time(actual: ${Math.floor(actualDuration / 1000)} sec, program: ${Math.floor(programDuration / 1000)} sec)`);
        }
    }
    ...
    async makeFolder() {
        ...
        const names = parsed.dir.replace(parsed.root, "").split(path.sep).filter(value => value !== "");

        let folder = parsed.root;
